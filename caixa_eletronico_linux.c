...
#include <string.h>
#include <time.h>
#include <termios.h>    // Controle de terminal (leitura de teclado sem eco)
#include <unistd.h>     // Funções do sistema Unix (sleep, read, STDIN)
...
void extratoTransacoes()
{
    ...
    printf("==============================================================================\n");
    printf("==============================================================================\n");
    printf("| DATA/HORA           | OPERAÇÃO             | VALOR       | SALDO           |\n");
    printf("==============================================================================\n");
...
void deposito()
{
    ...
    // Se acabou o limite de depósitos do dia
    if (quantidadeDeposito <= 0) {
        printf("\n[ERRO] Quantidade de depósitos diários atingida.\n\n");
        voltarMenu();
        return;
    }
    ...
    if (valor <= 0) {
        ...
        printf("\n[ERRO] Limite de depósito por operação (R$ 1000) excedido.\n\n");
    } else {
        // Atualiza o saldo após uma operação de depósito confirmada
        saldo += valor;
        quantidadeDeposito--;
        printf("\nDepósito realizado com sucesso!\n");
        printf("Depósitos restantes hoje: %d\n\n", quantidadeDeposito);
        logTransacao("Depósito", valor);    // Registra no extrato
...
void saque()
{
    ...
    if (quantidadeSaque <= 0) {
        printf("\n[ERRO] Quantidade de saques diários atingida.\n\n");
        voltarMenu();
        return;
    }
    ...
    if (valor <= 0) {
        ...
        printf("\n[ERRO] Limite de saque por operação (R$ 500) excedido.\n\n");
    } else if (valor > saldo) {
...
void menu()
{
    ...
    do {
        ...
        printf("2 - Saque\n"); 
        printf("3 - Depósito\n"); 
        printf("4 - Extrato de transações\n"); 
        printf("0 - Sair\n");
        printf("\nOperação desejada: ");
        if (scanf("%d", &opcao) != 1) {
            printf("\n[ERRO] Digite apenas números!\n\n");
...
char getch_linux()
{
    // Armazena as configurações do terminal
    struct termios oldt, newt;
    char num;

    // Salva as configurações atuais do terminal
    tcgetattr(STDIN_FILENO, &oldt);
    // Copia as configurações para fazer alterações temporárias
    newt = oldt;
    // Desativa a exibição das teclas digitadas e permite leitura imediata
    newt.c_lflag &= ~(ICANON | ECHO);
    // Aplica as novas configurações no terminal
    tcsetattr(STDIN_FILENO, TCSANOW, &newt);
    // Lê apenas uma tecla digitada pelo usuário
...
int lerPIN()
{
    ...
    while (1) {
        ...
        fflush(stdout);
        num = getch_linux();  // Captura a tecla sem exibi-la diretamente na tela
